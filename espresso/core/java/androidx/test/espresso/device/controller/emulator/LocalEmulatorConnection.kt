...
/**
 * A connection to the emulator in which this test is running.
 *
 * This class is able to produce fully configured channels, and call credential objects that can be
 * used for authenticating to the external emulator.
 *
...
constructor(
  ...
  override fun channel(): ManagedChannel {
    return OkHttpChannelBuilder.forAddress(EMULATOR_ADDRESS, port, channelCredentials).build()
  }
  ...
  override fun emulatorController(): EmulatorControllerGrpc.EmulatorControllerBlockingStub {
    return EmulatorControllerGrpc.newBlockingStub(channel()).withCallCredentials(credentials())
  }

  private fun channelCredentials(): ChannelCredentials {
    if (exists(clientCertChainFilePath) &&
